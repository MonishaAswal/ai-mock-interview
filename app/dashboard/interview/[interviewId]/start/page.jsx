...
const StartInterview = ({ params }) => {
  ...
  const GetInterviewDetails = async () => {
    try {
      ...
      // Defensively handle case where JSON response is an object wrapping an array
      if (jsonMockResp && typeof jsonMockResp === 'object' && !Array.isArray(jsonMockResp)) {
        if (Array.isArray(jsonMockResp.questions)) {
          jsonMockResp = jsonMockResp.questions;
        } else {
          const arrayKey = Object.keys(jsonMockResp).find(key => Array.isArray(jsonMockResp[key]));
          if (arrayKey) {
            jsonMockResp = jsonMockResp[arrayKey];
          }
        }
      }
